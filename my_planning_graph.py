...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        eff_a = self.children[actionA]
        eff_b = self.children[actionB]
        neg_a = set([~e for e in eff_a])
        neg_b = set([~e for e in eff_b])
        return neg_a.intersection(eff_b) or neg_b.intersection(eff_a)
    ...
    def _interference(self, actionA, actionB):
        ...
        eff_a = self.children[actionA]
        eff_b = self.children[actionB]
        neg_pre_a = set([~e for e in self.parents[actionA]])
        neg_pre_b = set([~e for e in self.parents[actionB]])
        return neg_pre_a.intersection(eff_b) or neg_pre_b.intersection(eff_a)
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        # TODO: implement this function
        pre_a = self.parents[actionA] 
        pre_b = self.parents[actionB] 
        return any([self.parent_layer.is_mutex(a, b) for a in pre_a for b in pre_b])
...
class LiteralLayer(BaseLiteralLayer):
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        """
        # TODO: implement this function
        act_a = self.parents[literalA]
        act_b = self.parents[literalB]
        return all([self.parent_layer.is_mutex(a, b) for a in act_a for b in act_b])
...
class PlanningGraph:
    ...
    def level_cost(self, goal):
        """Return the number of the literal layer in which the goal appears 
        first. 
...
        - goal, a literal that is a goal in the planning graph
    ...
    # def h_levelsum(self):
    #     self.fill() 
    #     costs = [self.level_cost(goal) for goal in self.goal]
    #     return sum(costs)
    ...
    def h_levelsum(self):
        ...
        while not self._is_leveled:
            met_here = unmet_goals.intersection(self.literal_layers[-1])
            unmet_goals -= met_here
            cost += i*len(met_here)
            if not unmet_goals:
                return cost
            else:
    ...
    def h_setlevel(self):
        ...
        for i, layer in enumerate(self.literal_layers):
            all_goals_met = all([g in layer for g in self.goal])
            if not all_goals_met:
                continue
            goals_are_mutex = any([layer.is_mutex(ga, gb) for ga in self.goal for gb in self.goal])
